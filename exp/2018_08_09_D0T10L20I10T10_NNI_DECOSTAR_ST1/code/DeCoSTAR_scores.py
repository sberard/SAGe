...
if len(parameters) != 3:
    print('usage: python linearity_score.py DIRECTORY PREFIX NNI_TREES_DIR')
    exit()
    
DIR  = parameters[0]
PREF = parameters[1]
DIR_NNI   = parameters[2]
...
print(PREF+'\t0\t'+str(nb_dup)+'\t'+str(nb_loss)+'\t'+str(nb_hgt)+'\t'+str(rec_score)+'\t'+str(nb_gain)+'\t'+str(nb_break)+'\t'+str(DeCo_score)+'\t'+str(linearity_score))
